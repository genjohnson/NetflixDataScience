...
def watched (List, Value ):
  for listValue in List:
    if listValue == Value: 
      return True
  return False

def top10( uID, numberOfMovies, valueList ):
    watchedMovie = []
    ...
    for line in f.readlines():
      ...
      if int(i) == uID:
        watchedMovie.append( int(j) ) 
    
    HitCt = 0		# counter for unwatched movie
    ct = 0		# counter to interate the valueList
    movieRefined = []	# recommendation list only containing unwatched movie
    while HitCt < numberOfMovies:
      if not watched( watchedMovie, valueList[ct] ):
        movieRefined.append( valueList[ct] )
        HitCt = HitCt + 1
      ct = ct + 1
    return movieRefined
...
def recommendMoviesForUser(uid, num_movies):
  ...
  top_recommendations = top10( uid, num_movies, mids_sorted )
  print(uid)
...
uID = 20
recommendMoviesForUser(uID, num_movies=50)
